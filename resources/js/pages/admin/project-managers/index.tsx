...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Project Managers', href: '/admin/project-managers' }];
...
export default function AdminProjectManagersIndex({ users, filters, vendors }: Props) {
    ...
    const updateFilter = (key: string, value: string) => {
        ...
        router.get(route('admin.project-managers.index'), newFilters, { preserveState: true });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Project Managers" />

            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">Project Managers</h1>
                        <p className="text-muted-foreground text-sm">Manager proyek yang bisa supervise tim & sub-vendor.</p>
                    </div>
                ...
                <div className="overflow-hidden rounded-lg border">
                    ...
                    {users.data.length === 0 && <div className="text-muted-foreground py-12 text-center">Belum ada project manager.</div>}
                </div>
            </div>
